...
### Nginx 代理https （demo11-https）

- 1 生成https 证书
在nginx目录下创建文件夹 certs，然后使用 git bash 打开命令行窗口（cmd 打开的命令行窗口下没有ssl）

openssl req -x509 -newkey rsa:2048 -nodes -sha256 -keyout localhost-privkey.pem -out localhost-cert.pem

运行成功后，生成 localhost-privkey.pem 和 localhost-cert.pem 2个文件

- 2  配置nginx
在test.conf 配置文件中（此处针对示例采用单独的配置，不在nginx.conf 中修改）
...
- 3 浏览器中访问  https://a.test.com/ 即可。

- 4 存在的问题

（1）实际运行时发现无法访问https://a.test.com/，经查找发现是 443 端口被占用，查找命令：
netstat -aon|findstr "443"，找到 0.0.0.0:443，找到 PID，在任务管理器结束进程

（2） nginx 的配置文件写错后，使用git bash的命令行执行./nginx.exe，可以看到控制台的错误，使用脚本启动看不到错误信息
...
http2 特点:
1. 信道复用
2. 分帧传输 把一个TCP连接 ,切分成有优先级的多个流, 可以双向发, 把数据切成更小的帧 用二进制编码, **不用在意顺序**，到了以后再组装起来, 可以按流的优先
3. 服务端推送（server push）：服务端可以主动推送